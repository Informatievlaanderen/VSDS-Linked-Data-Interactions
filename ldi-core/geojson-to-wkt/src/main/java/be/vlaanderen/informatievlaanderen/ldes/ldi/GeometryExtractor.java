...
public class GeometryExtractor {
    ...
    private MultiPoint createMultiPoint(Model model, Resource subject) {
        List<Coordinate> lineString = createLineStringCoordinates(model, subject, new ArrayList<>());
    ...
    private MultiLineString createMultiLineString(Model model, Resource subject) {
        List<List<Coordinate>> polygon = createPolygonCoordinates(model, subject, new ArrayList<>());
        LineString[] lineStrings = polygon.stream().map(ls -> ls.toArray(Coordinate[]::new))
    ...
    private MultiPolygon createMultiPolygon(Model model, Resource subject) {
        List<List<List<Coordinate>>> multiPolygon = createMultiPolygon(model, subject, new ArrayList<>());
    ...
    private List<List<Coordinate>> createPolygonCoordinates(Model model, Resource subject, List<List<Coordinate>> result) {
        Resource exteriorRing = model.listStatements(subject, RDF.first, (RDFNode) null).nextStatement().getObject()
                .asResource();
        List<Coordinate> exRing = new ArrayList<>();
        result.add(createLineStringCoordinates(model, exteriorRing, exRing));
        Resource nextRing = model.listStatements(subject, RDF.rest, (RDFNode) null).nextStatement().getObject()
                .asResource();
        if (RDF.nil.getURI().equals(nextRing.getURI())) {
            return result;
        } else {
            return createPolygonCoordinates(model, nextRing, result);
        }
    }
