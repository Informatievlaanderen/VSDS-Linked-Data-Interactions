...
import be.vlaanderen.informatievlaanderen.ldes.ldio.pipeline.creation.model.AdapterDebugger;
import be.vlaanderen.informatievlaanderen.ldes.ldio.pipeline.creation.model.ComponentExecutorImpl;
import be.vlaanderen.informatievlaanderen.ldes.ldio.pipeline.creation.model.LdiOutputLogger;
import be.vlaanderen.informatievlaanderen.ldes.ldio.pipeline.creation.model.LdioSender;
import be.vlaanderen.informatievlaanderen.ldes.ldio.pipeline.creation.model.OutputDebugger;
import be.vlaanderen.informatievlaanderen.ldes.ldio.pipeline.creation.model.TransformerDebugger;
...
public class PipelineCreatorService {
    ...
    public void initialisePipeline(PipelineConfig config) {
        try {
            ...
            resolvePropertyPlacerholders(config, inputConfig);
    ...
    private void resolvePropertyPlacerholders(PipelineConfig config, Map<String, String> inputConfig) {
        ...
        resolvePropertyPlaceHolders(resolver, config.getInput().getConfig());
        if (config.getInput().getAdapter() != null) resolvePropertyPlaceHolders(resolver, config.getInput().getAdapter().getConfig());
        config.getTransformers().forEach(map -> resolvePropertyPlaceHolders(resolver, map.getConfig().getConfig()));
        config.getOutputs().forEach(map -> resolvePropertyPlaceHolders(resolver, map.getConfig().getConfig()));
    }

    private void resolvePropertyPlaceHolders(PropertySourcesPlaceholdersResolver resolver, ComponentProperties componentProperties) {
        if (componentProperties.getConfig() != null) {
            resolvePropertyPlaceHolders(resolver, componentProperties.getConfig());
        }
    }

    private void resolvePropertyPlaceHolders(PropertySourcesPlaceholdersResolver resolver, Map<String, String> map) {
        map.replaceAll((key, value) -> resolver.resolvePlaceholders(value).toString());
    }
}
