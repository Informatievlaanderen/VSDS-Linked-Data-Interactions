...
public class PipelineManagementController {
	...
	public List<PipelineTO> overview() {
		return pipelineManagementService.getPipelines()
				.stream()
				.map(config -> fromPipelineConfig(config, pipelineStatusService.getPipelineStatus(config.getName()), pipelineStatusService.getPipelineStatusChangeSource(config.getName())))
				.toList();
	}
}
