...
class HttpInputPollerTest {
	...
	private final static String COMPONENT_POLLER = "HttpPoller";
    ...
    void setUp() {
		...
		httpInputPoller = new HttpInputPoller(COMPONENT_POLLER, pipelineName, executor, adapter, List.of(BASE_URL + ENDPOINT), true, noAuthExecutor);
    }
	...
	void whenPolling_andMissesHeader() {
		...
		httpInputPoller = new HttpInputPoller(COMPONENT_POLLER, pipelineName, executor, adapter, List.of(BASE_URL + ENDPOINT), false, noAuthExecutor);
	...
	void whenPollMultipleEndpoints_andOneEndpointFails_thenTheOtherEndpointShouldStillBePolled() {
		...
		httpInputPoller = new HttpInputPoller(COMPONENT_POLLER, pipelineName, executor, adapter, List.of(BASE_URL + ENDPOINT, BASE_URL + otherEndpoint),
				true, noAuthExecutor);
	...
	void when_EndpointDoesNotExist_Then_NoDataIsSent() {
		...
		httpInputPoller = new HttpInputPoller(COMPONENT_POLLER, pipelineName, executor, adapter, List.of(BASE_URL + wrongEndpoint), true,
				noAuthExecutor);
