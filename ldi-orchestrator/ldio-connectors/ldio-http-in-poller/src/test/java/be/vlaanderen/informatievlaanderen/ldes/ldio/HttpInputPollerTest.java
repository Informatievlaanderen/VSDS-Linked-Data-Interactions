...
class HttpInputPollerTest {
	private static final int WIRE_MOCK_PORT = 10101;
	@Rule
	...
	void setUp() throws IOException {
		...
		endpoint = String.format("http://localhost:%s", WIRE_MOCK_PORT);
	...
	void testClientPolling() throws InterruptedException {
		// mockBackEnd.enqueue(new MockResponse().addHeader("Content-Type",
		// CONTENT_TYPE).setBody(CONTENT));
	...
	void whenInvalidIntervalConfigured_thenCatchException(String interval) {
		Executable configurePoller = () -> scheduledExecutorService = new HttpInputPollerAutoConfig().httpInputPollerConfigurator().configure(adapter, executor, createConfig(endpoint, interval, "false"));
	...
	void whenPeriodicPolling_thenReturnTwoTimesTheSameResponse() throws InterruptedException {
		...
		scheduledExecutorService = new HttpInputPollerAutoConfig().httpInputPollerConfigurator().configure(adapter, executor, createDefaultTestConfig());
	...
	void when_OnContinueIsTrueAndPeriodPollingReturnsNot2xx_thenKeepPolling() {
		...
		scheduledExecutorService = new HttpInputPollerAutoConfig().httpInputPollerConfigurator().configure(adapter, executor, createConfig(endpoint, "PT1S", "true"));
	...
	void when_ResponseIsNot200_Then_NoDataIsSent() {
		...
		mockBackEnd.enqueue(new MockResponse().setResponseCode(405).addHeader("Content-Type", CONTENT_TYPE).setBody(CONTENT));
