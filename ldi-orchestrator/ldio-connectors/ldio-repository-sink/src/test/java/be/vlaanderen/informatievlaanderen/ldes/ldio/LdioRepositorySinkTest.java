...
class LdioRepositorySinkTest {
	...
	class ProcessList {
		...
		void given_ValidList_when_ProcessList_And_CommitFails_then_RollbackConnection() {
			SinkFailedException exception = new SinkFailedException(new RuntimeException());
			doThrow(exception).when(repositorySink).process(anyList());
			...
			for (int i = 0; i < models.size(); i++) {
				try {
					...
					assertThat(e).isInstanceOf(SinkFailedException.class);
					assertThat(i + 1)
