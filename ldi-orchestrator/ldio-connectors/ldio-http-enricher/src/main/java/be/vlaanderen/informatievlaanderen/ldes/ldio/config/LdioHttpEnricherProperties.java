package be.vlaanderen.informatievlaanderen.ldes.ldio.config;

public class LdioHttpEnricherProperties {

	private LdioHttpEnricherProperties() {
	}

	public static final String ADAPTER_NAME = "adapter.name";
	public static final String ADAPTER_CONFIG = "adapter.config";

	public static final String URL_PROPERTY_PATH = "url-property-path";
	public static final String HEADER_PROPERTY_PATH = "header-property-path";
	public static final String BODY_PROPERTY_PATH = "body-property-path";
	public static final String HTTP_METHOD_PROPERTY_PATH = "http-method-property-path";

}
