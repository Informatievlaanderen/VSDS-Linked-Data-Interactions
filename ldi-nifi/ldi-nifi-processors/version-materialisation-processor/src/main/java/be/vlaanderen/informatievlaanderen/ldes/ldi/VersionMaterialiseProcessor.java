...
import static be.vlaanderen.informatievlaanderen.ldes.ldi.config.VersionMaterialisationProcessorProperties.IS_VERSION_OF;
import static be.vlaanderen.informatievlaanderen.ldes.ldi.config.VersionMaterialisationProcessorProperties.RESTRICT_OUTPUT_TO_MEMBER;
	...
	public void onTrigger(final ProcessContext context, final ProcessSession session) {
		...
		if (flowFile != null) {
			try {
				...
				Model versionMaterialisedModel = versionMaterialiser.transform(inputModel);

				sendRDFToRelation(session, flowFile, versionMaterialisedModel, SUCCESS,
						RDFLanguages.contentTypeToLang(flowFile.getAttribute("mime.type")));
			} catch (Exception e) {
