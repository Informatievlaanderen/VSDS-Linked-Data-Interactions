...
class LdesClientStateTest {
	...
	void whenRunningOnTriggerOnce_thenExactlyOneImmutableFragmentIsPersisted() {
		testRunner.run(1);
		assertEquals(1, ((LdesClient) testRunner.getProcessor()).ldesService.getStateManager()
				.countProcessedImmutableFragments());
		assertEquals(2, ((LdesClient) testRunner.getProcessor()).ldesService.getStateManager().countProcessedMembers());

		testRunner.run(1);
		assertEquals(2, ((LdesClient) testRunner.getProcessor()).ldesService.getStateManager()
				.countProcessedImmutableFragments());
		assertEquals(4, ((LdesClient) testRunner.getProcessor()).ldesService.getStateManager().countProcessedMembers());

		testRunner.run(1);
		assertEquals(3, ((LdesClient) testRunner.getProcessor()).ldesService.getStateManager()
				.countProcessedImmutableFragments());
		assertEquals(6, ((LdesClient) testRunner.getProcessor()).ldesService.getStateManager().countProcessedMembers());
